...
<h4>Name: Malar Mariam S</h4>
<h4>Register Number: 212223230118</h4>
<h4>Ex no: 02</h4>
...
#include <stdio.h>
#include <sys/time.h>
#include <stdlib.h> // Required for abs() and malloc()/free()
...
void initialData(int *ip, const int size)
{
    ...
    for(i = 0; i < size; i++)
    {
        ip[i] = (int)(rand() & 0xFF); // Removed division by 10.0f
    }
...
{
    // double epsilon = 1.0E-8; // Not needed for integer comparison
    bool match = 1;

    for (int i = 0; i < N; i++)
    {
        // if (abs(hostRef[i] - gpuRef[i]) > epsilon) // Changed to direct integer comparison
        if (hostRef[i] != gpuRef[i])
        {
            match = 0;
            printf("host %d gpu %d at index %d\n", hostRef[i], gpuRef[i], i); // Added index for easier debugging
            break;
        }
    }
...
int main(int argc, char **argv)
{
    ...
    int nxy = nx * ny;
    // int nBytes = nxy * sizeof(float); // Changed to sizeof(int)
...
## OUTPUT:
<img width="844" height="245" alt="image" src="https://github.com/user-attachments/assets/da8a142c-73d6-4db4-9ced-ab6766e4e215" />


## RESULT:
The host took 22.5 seconds to complete it’s computation, while the GPU outperforms the host and completes the computation in ________ seconds. Therefore, float variables in the GPU will result in the best possible result. Thus, matrix summation using 2D grids and 2D blocks has been performed successfully.
